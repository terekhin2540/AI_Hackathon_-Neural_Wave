...
public protocol LoopingPlayerProtocol: AbstractPlayer, LayerMakerProtocol{
    ...
    var snapshotObserver: Any? { get set }
    ...
    init(asset: AVURLAsset, settings: VideoSettings, timePublishing: CMTime?, snapshotPublishing: CMTime?)
...
internal extension LoopingPlayerProtocol {
    ...
    func setupPlayerComponents(
        ...
        snapshotPublishing: CMTime?
    ) {
        
        let player = AVQueuePlayer(items: [])
        self.player = player
        
        configurePlayer(player, settings: settings, timePublishing: timePublishing, snapshotPublishing: snapshotPublishing)
            ...
            timeObserver = player.addPeriodicTimeObserver(forInterval: timePublishing, queue: .main) { [weak self] time in
                ...
                if let item = player.currentItem, let asset = item.asset as? AVURLAsset {
                    ...
                    if let thumb: CGImage = try? imageGenerator.copyCGImage(at: player.currentTime(), actualTime: nil) {
                        let myNsImage = NSImage(cgImage: thumb, size: .zero)
                        self.delegate?.didRecieveSnapshot(snapshot: myNsImage)
                    }
                }
            }
        }
        
        if let snapshotPublishing {
            snapshotObserver = player.addPeriodicTimeObserver(forInterval: snapshotPublishing, queue: .main) { [weak self] time in
